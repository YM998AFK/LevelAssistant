...
> 从 `SKILL.md` 剥离的**设计层面**硬规则汇总。每条都是踩坑立规的血泪实证。
> **开工前必读**，交付前对照自检。MUST/NEVER 是元级红线（见 SKILL.md 第零章），
...
```
grep "^\| {AssetId} \|" .cursor/skills/level-common/asset_catalog.md
```
...
> 完整规则与阵型模板见 `.cursor/skills/level-common/positioning.md`。以下为设计层核心约束。
