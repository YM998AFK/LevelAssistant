...
**第一步：逐行标注折叠区**（对每个 cpp 文件输出以下格式）

```
文件：挑战X答案页.cpp
折叠区A：[▼ 所在行内容] ~ [▲ 所在行内容]  →  区内代码：<逐行抄出>
折叠区B：[▼ 所在行内容] ~ [▲ 所在行内容]  →  区内代码：<逐行抄出>
```
...
1. `ListReplaceItemAt` 第3参数：`'1'` → `ListGetItemAt(1, cin_cut)`（外层循环跑 n 次）
2. 外层 `Repeat` 循环体末尾：新增 `BroadcastMessageAndWait('换行')`
3. `cmd=cin` 分支：删除 `BroadcastMessageAndWait('换行')`（换行已移入循环内）

**第三步：将折叠区分析结论纳入改动清单**

- 每条结构性改动（Repeat 次数/广播位置）标注「**因折叠边界移动**」
- 每条值类改动（演示值/SetVar参数）标注「**因折叠区内代码变化**」
...
当 C++ 代码中 `cin >>` 的**数量或语义**发生变化时，必须同步检查（若结构本身改变见 §7.3）：
...
### §7.3 cin_cut 输入协议变更（协议结构本身改变时）

当改关导致 C++ 输入从**一种结构**变为**另一种结构**（不只是数量，而是各索引的语义发生变化），需要做**完整的索引重映射**，而不是只增减 `ListAdd`。

**常见结构类型**

| 协议标识 | cin_cut 布局 | 典型场景 |
|---|---|---|
| `[n, x₁…xₙ]` | 首位是轮次 n，后续 n 个元素是数据 | 多轮输入（n 次循环，每次读一个 x） |
| `[x]` | 只有一个数据值 | 单轮单值（只读一次 cin） |
| `[x, y]` | 两个独立参数 | 双参数单轮 |
| `[n, x₁…xₙ, y₁…yₙ]` | 首位轮次，后续交叉或分组两组数据 | 多轮双参数 |

**重映射步骤（协议变更时必做）**

1. **确认新旧协议**：对照新 C++ 代码的 `cin >>` 顺序，写出新 cin_cut 布局表（索引 → 含义）
2. **找所有读取点**：在 S2b §5 和 §6 中搜索所有 `ListGetItemAt ... "cin_cut"` 调用，列出索引值
3. **逐一重映射**：按新布局更新每处索引；旧有的 `Repeat(cin_cut[1])` / `SetVar('sy_n', ...)` 等也一并检查
4. **更新 Demo fragment**：`WhenGameStarts` 里的 `ListAdd` 序列改为新协议的值
5. **更新 \*OJ-输入信息**：若有显式 `SetVar('*OJ-输入信息', ...)` 则按新格式修改；否则检查推断规则
6. **重新校验 cin_cut 注入**（见 §7.1 校验规则）

**典型案例：多轮 `[n, x₁…xₙ]` → 单值 `[x]`**

> 挑战12（`n` 次循环读 x）改为挑战9（只读一次 x）：
>
> | 位置 | 改前 | 改后 |
> |---|---|---|
> | `Repeat` 次数来源 | `cin_cut[1]`（即 n） | 固定 `'1'`（或直接展开不用 Repeat） |
> | 数据读取索引 | `cin_cut[i]`，i 从 2 起 | `cin_cut[1]`（x 直接在首位） |
> | Demo ListAdd | `ListAdd(n)` + `ListAdd(x₁)` … | `ListAdd(x)` 只注入一个值 |
> | 循环变量 | `sy_n = cin_cut[1]` | 不需要，删除或置常量 |

**注意**：演出 handler（`最终分数`、`开始攻击` 等）如果含有按索引读 cin_cut 的逻辑，**必须同步修改**；仅修改 Demo fragment 数据而不改 handler 会导致运行时越界或语义错乱。
...
- **v0.2（2026-05-09）**：新增 §7.3 cin_cut 输入协议变更规则（协议结构重映射步骤 + 多轮→单值典型案例）；§7.1 头部加交叉引用。
