...
## §10 · 多 Agent 协作坑（来自 `_shared/constraints.md` 延伸）
...
# 采样最近合法点（等同于 NavMesh.SamplePosition）
result = sample_scene_position(scene_id, [x, y, z], max_dist=2).to_dict()
# 查到边界距离（距离 < 1.0 时换点）
edge = find_scene_closest_edge(scene_id, [x, y, z]).to_dict()
```

放置路径顶点时确保：`edge["distance"] >= 1.0`（距 NavMesh 边界 ≥ 1.0 ws 单位）。

另：`validate_workspace` 会自动检测 `WalkToTarget` / `RunToTarget` 积木的目标落点合法性（scene_resource_id 存在时），无需手写校验脚本。`GotoPosition3D` / `GlideSecsToPosition3D` 等直接坐标移动积木不参与 NavMesh 检测。
...
- `_shared/constraints.md` —— 多 Agent 协作规则（含 M4/M7/N10 等全局约束）
...
- **v2（2026-05-09）**：§11.2 手写 navmesh 脚本替换为 MCP v0.0.4 `ws_update_scene_element_position` + `navigation.scene_navmesh` API；补充 `validate_workspace` 自动检测 WalkToTarget 目标合法性说明。
