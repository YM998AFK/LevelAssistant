...
> 动画列表已并入 `resource_index.jsonl` 的 `animations` 字段（383个资产，为本节超集）。
> 直接 Grep 搜索即可：`Grep pattern='"name":"队长"' path=_knowledge/resource_index.jsonl`
> 动画名含义（拼音→情绪/能力标签）见 `animation_dict.md`。
