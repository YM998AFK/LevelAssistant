# -*- coding: utf-8 -*-
"""
阶段 1: 用 scene_utils 准备好场景结构（角色/scene/control），
BlockScript 留空，等待后续 MCP add_fragment 写入积木。
"""
...
SCENE_ASSET_ID = 22422          # 藏画室
LEVEL_NAME     = "zanghuas_trio"
...
# 藏画室是大地图，BoundsCenter/BoundsSize 留引擎默认即可（引擎会按 AssetId 加载）
data["name"] = LEVEL_NAME
data["desc"] = "藏画室三角色站位 demo（NavMesh 驱动）"
data["modified"] = int(time.time())
print(f"[2] 场景 AssetId → {SCENE_ASSET_ID}")


# ── 3. 清空原有 Character / BlockScript / MeshPart，保留 Folder(services) ──
keep_types = {"Folder"}
...
print(f"[4] 场景摘要: islands={summary['island_count']}  area={summary['total_walkable_xz_area']:.0f}m^2")
print(f"    主岛重心: {summary['largest_island_centroid']}")

for seed in range(1, 50):
    pts = V.place_points_inside(SCENE_ASSET_ID, count=3, min_dist=2.0, margin=1.5,
                                island_index=0, seed=seed)
    if len(pts) == 3:
        print(f"    seed={seed} 拿到 3 个位置")
        break
else:
    raise RuntimeError("采样失败")

positions = [(CHAR_NAMES[i], pts[i].position) for i in range(3)]
...
rep = V.validate_positions(
    ...
    require_reachable_pairs=[(CHAR_NAMES[0], CHAR_NAMES[1]), (CHAR_NAMES[0], CHAR_NAMES[2])],
    min_spacing=2.0,
)
...
# ── 5. 放 control ────────────────────────────────────────────────────────
cx = sum(p[0] for _, p in positions) / 3
cy = positions[0][1][1]
cz = sum(p[2] for _, p in positions) / 3
...
# ── 6. 放角色（空 BlockScript，等 MCP add_fragment）──────────────────────
char_script_ids = {}
yaws = [90, 200, 310]
