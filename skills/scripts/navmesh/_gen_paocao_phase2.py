# -*- coding: utf-8 -*-
"""Phase 2: 打包 tiankong_paocao.zip（不覆盖 tiankong_quartet.zip）"""
...
if ICON_SRC.exists():
    ...
    import base64
    icon_dest.write_bytes(base64.b64decode(
...
solution = {
    ...
    "globals": [{
        ...
        "obj": {
            ...
            "children": [
                {"type": "BlockScript", "id": new_uuid32(),
...
(WORKDIR / "solution.json").write_text(
    json.dumps(solution, ensure_ascii=False, separators=(",",":")), encoding="utf-8")
(WORKDIR / "export_info.json").write_text(
    json.dumps({"solutionUid": solution_uid}, ensure_ascii=False, separators=(",",":")), encoding="utf-8")
