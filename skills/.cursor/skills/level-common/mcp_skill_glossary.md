# hetu-mcp ↔ level-common Skill 术语对照表

> **作用**：hetu-mcp 内部用工程侧命名（来自 Hetu 编辑器源码），level-common Skill 用业务侧命名（来自设计师/关卡文档）。两套术语指向同一个东西时容易互相混淆，本表做一次性映射。
>
...
| `modify_block_parameter(data, ...)` | 改积木参数槽 | **返回新 data**（函数式），见 `SKILL.md` 0.10.e 冲突 5 |
...
| "Position 是厘米" | **错**（中间一次搞错过，已作废） | 见 level-common `0.10.e` 和坐标系统第 0 条硬规则 |
